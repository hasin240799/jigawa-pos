...
export default function SalesView({ auth }) {
    ...
    const [statuses] = useState(['pending', 'completed', 'installment', 'paid']);
...
const formatNaira = (amount) => {
  ...
  /**
     ...
     */
  const handleCSVExport = (salesReport) => {
    if (!salesReport) {
    ...
    //   const dataToExport = salesReport.map(report => ({
    //     'Product Name': report.product_name,
    //     'Quantity Sold': report.quantity_sold,
    //     'Unit Price': formatNaira(report.selling_price),
    //     'Total Amount': formatNaira(report.selling_price * report.quantity_sold)
    // }));

      const dataToExport = salesReport.map(report => ({
        'Product Name': report.product_name || 'Dummy Product',
        'Quantity Sold': report.quantity_sold || 0,
        'Unit Price': formatNaira(report.selling_price || 0),
        'Total Amount': formatNaira((report.selling_price || 0) * (report.quantity_sold || 0))
    }));
    ...
    console.log(salesReport);
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Shops Sales Report"); 

   

      // Generate Excel file and download it
      XLSX.writeFile(wb, `${formData?.shop?.shopname ? formData?.shop?.shopname : 'All Reports'}-sales_report.xlsx`);
    } catch (error) {
    ...
    // });
        ...
        doc.save(`${formData?.shop?.shopname ? formData?.shop?.shopname : 'All Reports'}-sales_report-!.pdf`);
...
const  fetchProduct  = async ()=>{
 ...
 useEffect(() => {
     ...
     const  fetchReport  = async (shop)=>{
        try {
            const response = await axios.get('https://api.scan-verify.com/api/sales/report?shop_id='+shop,{
                ...
                console.log("Sales Data", response.data.report)
    ...
    const handleReceiptView = (rowData)=>{
            if(rowData.payment_type == 'card'){
                navigate(`/sales/receipt-card/${rowData.ref}?view=true`);
            }else if(rowData.payment_type == 'installment'){
...
const statusBody = (rowData) => {
    ...
    const header = (
        <div className="flex flex-row gap-x-4 items-center mx-auto my-auto justify-end pl-4">
          <Button type="button" icon="pi pi-file-pdf text-green-500 text-2xl" severity="warning" rounded onClick={exportPdf} data-pr-tooltip="PDF">Sales Report PDF</Button>
          <Button type="button" icon="pi pi-file-excel text-green-500 text-2xl" severity="warning" rounded onClick={()=>{handleCSVExport(salesReport)}} data-pr-tooltip="PDF">Sales Report Excel</Button>
...
<h1 className='text-gray-700 font-bold text-2xl'>Sales Records</h1>
