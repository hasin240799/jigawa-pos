...
export default function VerifyCustomer({ auth }) {
...
useEffect(() => {
    ...
    const verifyPSN = async () => {
        ...
        try {
            ...
            if (response.data.success == true) {
                ...
                setData(response.data.psn)
                    setFormData((prevState) => ({
                        ...prevState,
                        full_name: !response.data.psn.staff_name == null? response.data?.psn?.staff_name : '',
                    }));
     ...
     const  test  = async ()=>{
        try {
            const response = await axios.get('https://api.bizipay.ng/sanctum/csrf-cookie', {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
...
const PhotoBody = (rowData) => {
    return (
       <>
         <h1 className='font-bold text-green-500'>Customer Record Found</h1>
        <ul>
            <li><span>{rowData.full_name}</span></li>
            <li><span>{rowData.contact}</span></li>
        </ul>
        <Image src={rowData.passport} zoomSrc={rowData.passport} imageClassName=' w-40 h-30'  alt="Image" width="70" height="60" preview />
       </>
...
<DataTable
  ...
  tableStyle={{ minWidth: '60rem', height:"100%" }}
