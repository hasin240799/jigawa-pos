...
import { transformToNaira } from './../utils/transformToNaira.jsx';
...
const RealtimeSales = () => {
    ...
    const [revenue, setSumRevenue] = useState(0);
    const [qty, setSumQty] = useState(0);
    ...
    useEffect(() => {
        const fetchReport = async () => {
            try {
                ...
                const response = await axios.get('https://api.scan-verify.com/api/shop-sales-report', {
                ...
                const totalQty = data.reduce((sum, item) => sum + parseInt(item.quantity_sold), 0);

                // Update state with report data, total revenue, and total quantity
                setReport(data);
                setSumRevenue(totalRevenue);
                setSumQty(totalQty);
    ...
    <div className="container-fluid px-2 py-5">
       {loading && <Preloader/>} {/* Show Preloader while loading */}
        ...
        <div className='grid grid-cols-2 gap-4 justify-center mx-auto'>
            <div className=' bg-green-200 p-4 rounded-md  border w-60 h-auto grid grid-cols-3 '>
                <div className='bg-green-500  text-white p-3  rounded-lg justify-items-center'><i className='items-center pi pi-shopping-cart' style={{ fontSize: '2.0rem' }}></i></div>
                <div className='px-2 py-2  '>
                    <span className='flex flex-row font-extrabold font-sans text-1xl'>Quantity Sold</span>
                    <span className='flex flex-row text-gray-800 font-bold'>{qty}</span>
                </div>
            </div>
            <div className=' bg-green-200 p-4 rounded-md  border w-60 h-auto grid grid-cols-3 '>
                <div className='bg-green-500  text-white p-3  rounded-lg justify-items-center'><i className='items-center pi pi-money-bill' style={{ fontSize: '2.0rem' }}></i></div>
                <div className='px-2 py-2  '>
                    <span className='flex flex-row font-extrabold font-sans text-1xl'>Revenue</span>
                    <span className='flex flex-row text-gray-800 font-bold'>{transformToNaira(revenue)}</span>
                </div>
            </div>
        </div>

        <ResponsiveContainer width="100%" height={300} className="mx-auto mb-12">
             {/* Title Above the Chart */}
             <h2 className="text-2xl font-semibold text-gray-800 text-center my-4">Top Sold Products</h2>
                ...
                <PieChart width={400} height={400}>
                    <Pie
                        ...
                        cy="50%"
                        outerRadius={120}
                        innerRadius={60} // Add inner radius for a donut effect
                        fill="#8884d8"
                        labelLine={false} // Disable label lines
                        label={({ name, value }) => `${name}: ${value}`}
                    >
                        {totalProductsData.map((entry, index) => (
            ...
            <ResponsiveContainer width="100%" height={300} className=" mx-auto my-4">
            <BarChart data={shopSalesData}>
                <XAxis dataKey="shopName"/>
                <YAxis />
                <Tooltip />
                <Legend />
                <Bar dataKey="sales" fill="#4d8" />
            </BarChart>
            </ResponsiveContainer>
        </div>

        <ResponsiveContainer width="100%" height={300}>
            <LineChart data={salesByTimeData}>
                <XAxis dataKey="time" />
                <YAxis />
                <Tooltip />
                <Line type="natural" dataKey="sales" stroke="#4d8" />
            </LineChart>
            </ResponsiveContainer>
