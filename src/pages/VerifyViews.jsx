...
export default function VerifyView({ auth }) {
    ...
    const [fetchSale,SetFetchSale] = useState([]);
    ...
    useEffect(() => {
     ...
     const  fetchSales  = async ()=>{
        try {
            const response = await axios.get('https://api.scan-verify.com/api/sales/getSales', {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
...
const statusBody = (rowData)=>{
    return(
        <>
       <span>{getval(rowData.picked_status)}</span>
        </>
    )
}
        ...
        <>
            <ul>
                {rowData.products.map((p,index)=>(
                    <li>{index+1} . {p.name} - {p.qty} - {p.selling_price}</li>
                ))
...
<h1 className='text-gray-700 font-bold text-2xl'>Picked products</h1>
