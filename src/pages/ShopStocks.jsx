...
export default function ShopStocks({ auth }) {
    ...
    //   const handleExport = () => {
    //     if (!stocks) {
    //       console.error('Error exporting data: stocks is null or undefined');
    //       return;
    //     }

    //     try {
    //       // Extract the relevant fields from the loans data
    //       const dataToExport = stocks.map((data, index) => ({
    //         "id": index,
    //         "Products": data?.product?.name || '',
    //         "Purchase Price": data?.product?.purchase_price || 0,
    //         "Selling Price": data?.product?.selling_price || 0,
    //         "Quantity In": data?.qty_in || 0,
    //         "Quantity Out": data?.qty_out || 0,
    //         "Stock Balance": (data.qty_in) - (data?.qty_out) || 0,
    //         "Shops": data?.shop?.shopname || '',
    //       })); 
    ...
    //       XLSX.writeFile(wb, `Reports-Stocks.xlsx`);
    //     } catch (error) {
    ...
    //     try {
    //         // Group stocks by shop
    //         const groupedByShop = stocks.reduce((acc, data) => {
    //             const shopName = data?.shop?.shopname || 'Unknown Shop'; // Use a default name if shop is not defined
    //             if (!acc[shopName]) {
    //                 acc[shopName] = [];
    //             }
    //             acc[shopName].push(data);
    //             return acc;
    //         }, {});
    ...
    //         // Iterate over each shop and create a separate sheet
    //         for (const [shopName, shopStocks] of Object.entries(groupedByShop)) {
    //             // Extract relevant fields for the current shop
    //             const dataToExport = shopStocks.map((data, index) => ({
    //                 "id": index,
    //                 "Products": data?.product?.name || '',
    //                 "Purchase Price": data?.product?.purchase_price || 0,
    //                 "Selling Price": data?.product?.selling_price || 0,
    //                 "Quantity In": data?.qty_in || 0,
    //                 "Quantity Out": data?.qty_out || 0,
    //                 "Stock Balance": (data.qty_in || 0) - (data.qty_out || 0),
    //             }));
    
    //             // Create a worksheet for the current shop
    //             const ws = XLSX.utils.json_to_sheet(dataToExport);
    //             // Append the worksheet to the workbook with the shop name
    ...
    const handleExport = () => {
        ...
        // Check if stocks is an object
        if (typeof stocks !== 'object' || Array.isArray(stocks)) {
            console.error('Error exporting data: stocks is not an object', stocks);
            return;
        }
        ...
        try {
            ...
            // Iterate over each shop and create a separate sheet
            for (const [shopName, shopStocks] of Object.entries(stocks.shop)) {
                // Ensure shopStocks is an array
                if (!Array.isArray(shopStocks)) {
                    console.error(`Error: Expected shopStocks to be an array for shop ${shopName}`, shopStocks);
                    continue; // Skip this shop if it's not an array
                }
    
                // Extract relevant fields for the current shop
                const dataToExport = shopStocks.map((data, index) => ({
                    "id": index + 1, // Starting from 1 for better readability in Excel
                    "Products": data?.product?.name || '',
                    "Purchase Price": data?.product?.purchase_price || 0,
                    "Selling Price": data?.product?.selling_price || 0,
                    "Quantity In": data?.qty_in || 0,
                    "Quantity Out": data?.qty_out || 0,
                    "Stock Balance": (data.qty_in || 0) - (data.qty_out || 0),
                    "Current Total Amount": new Intl.NumberFormat('en-US').format(data?.qty_in * data?.product?.selling_price),
                    "Expected Total Amount": new Intl.NumberFormat('en-US').format((data?.qty_in - data?.qty_out) * data?.product?.selling_price),
                }));
                ...
                const headerCells = ['A1', 'B1', 'C1', 'D1', 'E1', 'F1', 'G1', 'H1']; // Adjust based on the number of columns
                ...
                Object.keys(dataToExport[0]).forEach((key) => {
                    colWidth[key] = { wpx: 100 }; // Default width
                });
    
                dataToExport.forEach((row) => {
                    Object.keys(row).forEach((key) => {
                        const cellValue = row[key]?.toString() || '';
                        const currentWidth = colWidth[key]?.wpx || 100; // Get current width
    
                        // Update width based on the longest value (header and data)
                        colWidth[key].wpx = Math.max(currentWidth, cellValue.length * 10); // 10 is a rough estimate for character width
                    });
                });
    
                // Set the column widths to the worksheet
                ws['!cols'] = Object.keys(colWidth).map((key) => colWidth[key]);
  ...
  const handlePDFExport = () => {
    ...
    try {
        ...
        // Iterate over each shop and add its data to the PDF
        for (const [shopName, shopStocks] of Object.entries(stocks.shop)) {
            // Ensure shopStocks is an array
            if (!Array.isArray(shopStocks)) {
                console.error(`Error: Expected shopStocks to be an array for shop ${shopName}`, shopStocks);
            ...
            // Set a new page for each shop (only if more than one shop exists)
            ...
            // Add shop name as a header in the PDF
            const yPosition = 50; // Reset yPosition to the top of the page for each shop
            pdf.setFont("helvetica", "bold");  // Set bold font style
            pdf.setFontSize(12);
            pdf.text(`Stock Report for ${shopName}`, (pdf.internal.pageSize.width/2)-20, pdf.internal.pageSize.height/2);
            pdf.text(`Date Generated ${date}`, yPosition+10, 10);
            ...
            autoTable(pdf, {
                ...
                body: dataToExport.map(row => [
                    ...
                    row['Quantity Out'],
                    row['Stock Balance'],
                    row['Current Total Amount'],
                    row['Expected Total Amount'],
                ]),
                styles: {
                ...
                theme: 'grid',
                margin: { top: 20 },
                pageBreak: 'avoid' // Prevent auto page-breaks for this table
            });
        }

        // Save the PDF with a common filename
...
const balanceBody= (rowData)=>{
    ...
    const header = (
        <div className="flex items-center mx-auto my-auto justify-end pl-4">
          ...
          <Button type="button" icon="pi pi-file-excel text-green-500 text-2xl" severity="warning" rounded onClick={handleExport} data-pr-tooltip="PDF">Download Stock Reports Excell</Button>
          <Button type="button" icon="pi pi-file-pdf text-green-500 text-2xl" severity="warning" rounded onClick={handlePDFExport} data-pr-tooltip="PDF">Download Stock Reports PDF</Button>
{/*
          <span className="">
             <button
...
<h1 className='text-gray-700 font-bold text-2xl'>Shops Stock</h1>
