    ...
    useEffect(() => {
     ...
     const verifyPayment  = async (saleRefs)=>{
        try {
            ...
            setProcessingStatus(response.data.processingStatus == 'PROCESSED'? 'APPROVED': 'PENDING')
    ...
    const qtyBody = (rowData) => {
      ...
      const statusBody = (processingStatus)=>{       
        switch (processingStatus) {
            ...
            return(
                <>
                  <span className=" font-bold mt-0 font-serif" style={{ fontSize: '15px',fontFamily:'bolder', color:'red' }}>{processingStatus.toUpperCase()}</span><br/>
                </>
             )
            default:
                return(
                    <>
                      <span className=" font-bold mt-0 font-serif" style={{ fontSize: '15px',fontFamily:'bolder', color:'red' }}>Unsuccessful</span><br/>
                    </>
                 )
                break;
        }
