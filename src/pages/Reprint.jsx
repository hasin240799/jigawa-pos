...
export default function Reprint() {
    ...
    const  fetch  = async ()=>{
     ...
     const verifySale = async () => {
        ...
        try {
            ...
            const response = await axios.get(`https://api.scan-verify.com/api/sales/verify/${saleRefs}`, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
    ...
    const  fetchSale  = async ()=>{
        try {
            const response = await axios.get(`https://api.scan-verify.com/api/sales/${saleRefs}`, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
...
const handleVerify = ()=>{
    ...
    return (
       ...
       <div className='border border-spacing-1 border-green-200 bg-green-100 mt-4 text-green-900 rounded-md  px-2 py-3'>
            ...
            <ConfirmDialog />
                ...
                <InputField
                    ...
                    onChange={(e)=>{setSaleRefs(e.target.value)}}
                />
       ...
       <div className='border border-spacing-1 gap-2 grid grid-cols-2 px-2'>
            ...
            <button onClick={handleVerify} className='bg-green-500 hover:bg-green-900 ml-3 text-white mt-5 w-1/2 rounded-md grid grid-cols-2  justify-center items-center'>
                < FaThumbsUp/>
                <span>Verify</span>
            </button>
        </div>
...
<div className="flex items-center  print:w-auto print:h-auto justify-center p-4">
        <div id="receipt-print" className=" print:w-auto print:h-auto bg-white  p-4  rounded-md font-mono">
                ...
                <div style={{ fontSize: '12px' }}>***********************</div>
                    <span className=" font-bold mt-0" style={{ fontSize: '15px',color:'red',fontWeight:'bolder' }}>{status(productData?.sale?.picked_status)}</span><br/>
                <div style={{ fontSize: '12px' }}>************************</div>
                ...
                <span className=' text-1xl' style={{fontSize:'12px'}}><b>Date:</b>{formatDateTime(productData?.sale?.created_at)}</span>
                <table className='w-full border-y border-collapse border-black'>
                    ...
                    {sale.map((product, index) => (
                        <tr key={index} style={{ fontSize: '12px' }}>
                            ...
                            <td className='text-right px-1 py-1'>{new Intl.NumberFormat('en-US', {style: 'currency',
                                                currency: 'NGN', // or 'NGN' for Nigerian Naira
                                                }).format(product.selling_price)}</td>
                        </tr>
                    ))}

                    <tr className=' border-y-black border-y-1' style={{ fontSize: '12px' }}>
                            <td className='text-left px-2 py-1'></td>
                            <td className='text-left px-2 py-1 font-bold text-1xl'>Total</td>
                            <td className='text-right px-1 py-1 font-bold text-1xl'>{new Intl.NumberFormat('en-US', {style: 'currency',
                                                currency: 'NGN', // or 'NGN' for Nigerian Naira
                                                }).format(total)}</td>
                        </tr>
                    </tbody>
                ...
                <div className='font-bold font-medium text-center mx-auto'>wwww.bizipay.ng</div>
                        {/* <canvas ref={inputRef} /> */}
            </div>
        </div>
{/*
